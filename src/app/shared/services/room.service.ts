...
export class RoomService {
    ...
    addItemToRoom(type: TypeRoomItem): void {
        ...
        const id =
            this.roomSelected.roomItems.length > 0
                ? this.roomSelected.roomItems[
                      this.roomSelected.roomItems.length - 1
                  ].id + 1
                : 1;
        ...
        if (this.roomSelected.roomItems.length > 0) {
            ...
            while (found) {
                ...
                this.roomSelected.roomItems.forEach((room) => {
                    if (
                        room.position.x === newPosition.x &&
                        room.position.y === newPosition.y
                    ) {
                        isNotExist = false;
                    }
                });
    ...
    async addRoom(name: string): Promise<void> {
        const id =
            this.rooms.length
                ? this.rooms[this.rooms.length - 1].id + 1
                : 1;
        ...
        this.roomSelected = this.rooms[this.rooms.length - 1];
        this.updateRoomsLocalStorage();
